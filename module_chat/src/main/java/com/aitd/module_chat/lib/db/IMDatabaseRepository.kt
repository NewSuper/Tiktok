package com.aitd.module_chat.lib.db

import com.aitd.library_common.utils.GlobalContextManager
import com.aitd.module_chat.pojo.MessageType
import com.aitd.module_chat.lib.db.dao.*
import com.aitd.module_chat.lib.db.entity.*
...
class IMDatabaseRepository {
    ...
    fun calcUnTrustTime(conversationType: String, targetId: String, min: Long, max: Long) {
        ...
        var list = unTrustTimeDao.containsWithAndUpdate(conversationType, targetId, UserInfoCache.getUserId(), min, max)
...
//                }
            ...
            if (messageEntity.retransmissionMessage != null) {
                messageEntity!!.retransmissionMessage!!.conversationId = messageEntity.conversationId
    ...
    fun markMessageDelete(messageIds: Array<String>): Int {
        var result = messageDao!!.markMessageDelete(messageIds.toList(), 1, UserInfoCache.getUserId())
//        for (index in messageIds.indices) {
//            result += messageDao!!.markMessageDelete(messageIds[index], 1)
//        }
        return result
    }
...
//        conversationDao!!.updateTimeIndicator(conversationId, -1, -1,UserInfoCache.getUserId())
    ...
    fun deleteLocalMessageByTimestamp(conversationType: String, targetId: String, timestamp: Long): Int {
        ...
        return messageDao!!.deleteMessageByTimestamp(UserInfoCache.getUserId(), timestamp, conversation.conversationId)
    }
        ...
        when (messageEntity.messageType) {
           MessageType.TYPE_AUDIO -> {
                return audioMessageDao!!.updateOriginUrl(
                    messageEntity.messageId, messageEntity.audioMessage!!.originUrl
                )
            }
           MessageType.TYPE_VIDEO -> {
                return videoMessageDao!!.updateOriginUrl(
                    messageEntity.messageId, messageEntity.videoMessage!!.originUrl
                )
            }
            MessageType.TYPE_IMAGE -> {
                return imageMessageDao!!.updateOriginUrl(
                    messageEntity.messageId, messageEntity.imageMessage!!.originUrl
                )
            }
            MessageType.TYPE_FILE -> {
                return fileMessageDao!!.updateOriginUrl(
                    messageEntity.messageId, messageEntity.fileMessage!!.originUrl
                )
            }
            MessageType.TYPE_GEO -> {
                return geoMessageDao!!.updatePreviewUrl(
                    messageEntity.messageId, messageEntity.geoMessage!!.previewUrl
                )
            }
            else -> {
                return 0
    ...
    fun updateCustomMessage(conversationId: String, messageId: String, content: String, extra: String): Int {
    ...
    fun updateAllMessageReadState(sendType: String, to: String): Int {
        return messageDao!!.updateAllMessageStateToRead(
            sendType, UserInfoCache.getUserId(), to, MessageEntity.State.STATE_READ, UserInfoCache.getUserId()
        )
    }
    ...
    fun updateMessageStateByConversationId(conversationId: String, state: Int): Int {
        return messageDao!!.updateMessageStateByConversationId(conversationId, state, UserInfoCache.getUserId())
    }
    ...
    fun updateP2PTopTimeAndTimeIndicator(sendType: String, targetId: String, topTime: Long, timeIndicator: Long): Int {
        return conversationDao!!.updateTimeIndicator(sendType, targetId, topTime, timeIndicator, UserInfoCache.getUserId())
    }

    fun updateGroupTopTimeAndTimeIndicator(sendType: String, from: String, topTime: Long, timeIndicator: Long): Int {
        return conversationDao!!.updateGroupTimeIndicator(sendType, from, UserInfoCache.getUserId(), topTime, timeIndicator)
    }

    fun updateTimeIndicator(conversationId: String, timeIndicator: Long): Int {
        return conversationDao!!.updateTimeIndicator(conversationId, timeIndicator, UserInfoCache.getUserId())
    }
    ...
    fun getLatestMessage(sendType: String, from: String, to: String, ownerId: String): MessageEntity? {
        if (sendType.isNullOrEmpty() || to.isNullOrEmpty() || ownerId.isNullOrEmpty()) {
            return null
        }
        var messageEntity = messageDao!!.getLatestMessage(sendType, from, to, ownerId)
    ...
    fun checkSendingMessage(): List<MessageEntity> {
        var list = messageDao!!.getMessageByState(MessageEntity.State.STATE_SENDING, UserInfoCache.getUserId())
        for (message in list) {
    ...
    fun getMessagesByTimestamp(conversationType: String, targetId: String, timestamp: Long, searchType: Int, pageSize: Int): List<MessageEntity>? {
        var list = arrayListOf<MessageEntity>()

        var conversation = conversationDao!!.getConversationByTargetId(conversationType, targetId, UserInfoCache.getUserId())
        var conversationId = ""
        if (conversation != null) {
        ...
        //从本地获取数据
        list = getLocalMessageByTimestamp(conversationId, timestamp, pageSize, searchType) as ArrayList<MessageEntity>
     ...
     */
    fun getMessages(conversationType: String, targetId: String, offset: Int, pageSize: Int): List<MessageEntity>? {
        var list = arrayListOf<MessageEntity>()
     ...
     * 计算数据连续性，结合多段时间窗
     */
    private fun calcData(list: ArrayList<MessageEntity>, conversationId: String, conversationType: String, targetId: String,
                         pageSize: Int, searchType: Int) {
        if (!list.isNullOrEmpty()) {
            ...
            if (list.size == pageSize) {
                //判断该页的数据是否跨不信任时间区域
                var startTime = unTrustTimeDao.startsWith(conversationType, targetId, UserInfoCache.getUserId(), min, max)
                var endTime = unTrustTimeDao.endsWith(conversationType, targetId, UserInfoCache.getUserId(), min, max)
...
//                getHistoryMessage(conversationType, targetId, min)
    ...
    fun addConversationUnReadCount(sendType: String, from: String, to: String, count: Int,mentionedCount:Int): Int {
        var targetId = TargetIdUtil.getTargetId(sendType, from, to)
        return conversationDao!!.addConversationUnReadCount(sendType, targetId, count, UserInfoCache.getUserId(),mentionedCount)
    }

    fun updateConversationUnReadCount(sendType: String, targetId: String, count: Int,mentionedCount:Int): Int {
        return conversationDao!!.updateConversationUnReadCount(sendType, targetId, count, UserInfoCache.getUserId(),mentionedCount)
    }
    ...
    fun getHistoryMessage(conversationType: String, targetId: String, timestamp: Long, searchType: Int) {
        QLog.d(TAG,  "从服务器上获取消息记录： conversationType=$conversationType targetId=$targetId timestamp=$timestamp"
        )

        var body = C2SMessageLoad.MessageLoad.newBuilder().setSendType(conversationType).setSearchType(searchType).setTargetId(targetId)
    ...
    fun searchConversations(keyword: String, conversationTypes: Array<String>, messageTypes: Array<String>): List<SearchConversationResult> {
        ...
        for (conversation in conversations) {
            var searchConversationResult =
                SearchConversationResult(
                    ConversationUtil.toConversation(conversation), 0
                )
                ...
                MessageType.TYPE_TEXT -> {
                    ...
                    if (matchCounts != null) {
                        for (count in matchCounts) {
                            ...
                            var searchConversationResult =
                                SearchConversationResult(
                                    ConversationUtil.toConversation(conversation),
                                    count.matchCount
                                )
                            results.add(searchConversationResult)
                        }
    ...
    fun getConversation(conversationType: String, targetId: String): ConversationEntity? {
        return conversationDao!!.getConversationByTargetId(conversationType, targetId, UserInfoCache.getUserId())
