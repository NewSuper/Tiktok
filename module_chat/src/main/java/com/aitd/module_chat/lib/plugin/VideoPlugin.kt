...
class VideoPlugin : IPluginModule {
    override fun obtainDrawable(context: Context): Drawable {
        return context.resources.getDrawable(R.drawable.vector_video_call)
    }

    override fun obtainTitle(context: Context): String {
        return context.resources.getString(R.string.qx_chat_add_panel_video_call)
    }
    ...
    override fun onClick(context: Activity, extension: QXExtension) {
        if (extension.conversationType == ConversationType.TYPE_GROUP) {
            ...
            QXCallKit.startMultiCall(context,extension.conversationType,extension.targetId,memebers,QXCallKit.CallMediaType.CALL_MEDIA_TYPE_VIDEO)
        } else {
            QXCallKit.startSignleCall(context,extension.conversationType,extension.targetId,QXCallKit.CallMediaType.CALL_MEDIA_TYPE_VIDEO )
        }
    }
