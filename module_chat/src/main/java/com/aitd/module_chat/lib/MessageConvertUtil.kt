...
import com.aitd.module_chat.pojo.json.*
...
class MessageConvertUtil {
    ...
    /**
     * 将probo的msg转为MessageEntity（消息内容一起转）
     */
    fun convertToMessageEntity(msg: S2CCustomMessage.Msg): MessageEntity {
        val entity = MessageEntity.obtain(msg.sendType, msg.messageType, msg.from, msg.to)
        ...
        if (entity.from == UserInfoCache.getUserId()) {
            ...
            entity.direction = MessageEntity.Direction.DIRECTION_RECEIVED
        }
    ...
    /**
     * 将服务器返回的msg转为客户端使用的MessageEntity
     */
    private fun parseMessageContent(entity: MessageEntity, msg: S2CCustomMessage.Msg): MessageEntity {
        ...
        when (jsonMessage.messageType) {
           ...
           MessageType.TYPE_VIDEO -> {
                if (jsonMessage.video != null) {
                    ...
                    var video = VideoMessage(
                        "", jsonVideo!!.duration, jsonVideo.size, jsonVideo.headUrl,
           ...
           MessageType.TYPE_AUDIO_CALL -> {
                if (jsonMessage.act != null) {
                    ...
                    var audio = CallMessage(
                        "", CallMessage.CallType.CALL_TYPE_AUDIO,
          ...
          MessageType.TYPE_VIDEO_CALL -> {
                if (jsonMessage.vct != null) {
                    ...
                    var video = CallMessage(
                        "", CallMessage.CallType.CALL_TYPE_VIDEO,
    ...
    fun s2cSndMsg(msg: S2CCustomMessage.Msg): Message? {
    ...
    private fun setBody(s2CSndMessage: S2CSndMessage, message: Message) {
        val builder = S2CCustomMessage.Msg.newBuilder()
        builder.setSendType(message.conversationType).setMessageId(message.messageId).setFrom(message.senderUserId)
        ...
        when (message.messageContent) {
            is TextMessage -> {
                ...
                if (textMessage.atToMessageList != null && textMessage.atToMessageList.size > 0) {
            ...
            is AudioMessage -> {
                ...
                val audio =
                    S2CCustomMessage.Audio.newBuilder().setOriginUrl(audioMessage.originUrl).setSize(audioMessage.size)
                        .setDuration(audioMessage.duration).setExtra(audioMessage.extra).build()
                builder.audio = audio
            }
            is VideoMessage -> {
    ...
    fun messageToMessageEntity(message: Message): MessageEntity {
        ...
        when (message.messageContent) {
            is TextMessage -> {
                ...
                val list: MutableList<TBAtToMessage> = ArrayList()
