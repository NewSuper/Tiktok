...
import com.aitd.module_chat.lib.handler.*
...
class CmdDispatcher {
    ...
    private val mHandlerMap: HashMap<Short, BaseCmdHandler?> = HashMap()
    ...
    fun dispatch(ctx: ChannelHandlerContext, data: Any) {
        try {
            ...
            QLog.e(TAG, e.message +" "+e.cause )
            e.printStackTrace()
        }
    }

    private fun getHandler(cmd: Short): BaseCmdHandler {
        if(cmd == SystemCmd.C2S_SEND_GROUP_MESSAGE) {
            return ReceiveGroupMessageHandler()
        } else {

            var handler = mHandlerMap[cmd]
            if (handler == null) {
                //如果handler map里找不到，则需要添加
                val clazz = mClassMap[cmd]
                handler = if (clazz == null) {
                    //如果找不到该handler类型，则按DefaultHandler处理
                    DefaultHandler()
                } else {
    ...
    private fun answerAck(cmd: Short, recMessage: S2CRecMessage, ctx: ChannelHandlerContext) {
        //如果cmd为ACK或者认证成功，则不需要回ACK，否则需要回复ack
        ...
        if (cmd != SystemCmd.S2C_ACK && cmd != SystemCmd.S2C_AUTH_SUCCESS) {
            ...
            QLog.w(TAG, "给服务器回复ack sequence:" + recMessage.sequence)
        }
    }
