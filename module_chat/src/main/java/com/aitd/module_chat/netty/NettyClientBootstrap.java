...
public class NettyClientBootstrap {
    ...
    public void sendMessage(S2CSndMessage message) {
        try {
            QLog.d(TAG, "发送消息 cmd：" + message.getCmd() + " sequence=" + message.getSequence());
            if (message.getBody() != null) {
                QLog.d(TAG, " message:" + message.getBody().toString().replace("\n",
                        ","));
            }
            ...
            QLog.e(TAG, "sendMessage error:" + e.getMessage() + " cause:" + e.getCause());
            EventBusUtil.post(ConnectionStatusListener.Status.DISCONNECTED);
            e.printStackTrace();
        }
    }
    ...
    public void connectNetty(String serverHost, int serverPort) {
        QLog.d(TAG,
                "尝试建立tcp长连接 connect > serverHost=" + serverHost + " serverPort=" + serverPort);
...
//            EventBusUtil.post(ReconnectEvent.EVENT_RECONNECT);
    ...
    ChannelFutureListener mChannelFutureListener = new ChannelFutureListener() {
        ...
        public void operationComplete(ChannelFuture future) throws Exception {
            QLog.e(TAG, "operationComplete 建立tcp长连接：" + future.isSuccess());
            if (future.isSuccess()) {
//                getAesKey();
                //生成公私钥对
