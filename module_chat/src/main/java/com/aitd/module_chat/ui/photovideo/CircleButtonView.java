...
public class CircleButtonView extends View {
    ...
    //获取设置的录制最大时间s
    private long maxRecordTime =  QXConfigManager.getQxFileConfig().getVideoShotMaxDuration(TimeUnit.SECONDS);
    //获取设置的录制最短时间s
    private long minRecordTime = QXConfigManager.getQxFileConfig().getVideoShotMinDuration(TimeUnit.SECONDS);
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mProgressAni.setDuration(maxRecordTime * 1000);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mEndTime - mStartTime < mLongClickTime) {
                    ...
                    if (mProgressAni != null && mProgressAni.getCurrentPlayTime() / 1000 < minRecordTime && !isMaxTime) {
                        if (onLongClickListener != null) {
                            onLongClickListener.onNoMinRecord(minRecordTime);
                        }
                        mProgressAni.cancel();
                    } else {
                        //录制完成
